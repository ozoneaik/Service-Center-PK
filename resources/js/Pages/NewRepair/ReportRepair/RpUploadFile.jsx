...
export default function RpUploadFile({detail, job_id, productDetail, jobDetail}) {
    ...
    const fetchMenu = async () => {
        try {
            ...
            const response = await axios.get(route('repair.upload-file.detail', {job_id}))
            setSelected(response.data.file_upload)
            setLoading(false);
        } catch (error) {
                ...
                <Grid2 container spacing={4}>
                    ...
                    <Grid2 size={12}>
                        ...
                        >
                            <Button
                                variant='outlined'
                                disabled={jobDetail.job_detail.status === 'success'}
                            >
                                ยกเลิก
                            </Button>
                            <Button
                                onClick={handleSave}
                                disabled={jobDetail.job_detail.status !== 'pending'}
                                variant='contained'
                            >
                                บันทึก
                            </Button>
