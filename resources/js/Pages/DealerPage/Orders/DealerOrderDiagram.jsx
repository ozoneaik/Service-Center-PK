...
import CartDrawer from "@/Pages/Orders/carts/CartList.jsx";
...
// dealer-specific endpoints (ราคา disc40p เสมอ)
const DEALER_SEARCH_SP_URL = "/dealer-repair/orders/search-sp";
const DEALER_CART_JSON_URL = "/dealer-repair/orders/carts/json";
const DEALER_ADD_CART_URL  = "/dealer-repair/orders/carts/add-cart";
const DEALER_DIAGRAM_ROUTE = "/dealer-repair/orders";
...
export default function DealerOrderDiagram() {
    ...
    async function loadDiagram(sku, dm) {
        ...
        try {
            const [diagRes, priceRes] = await Promise.allSettled([
                ...
                axios.get(DEALER_SEARCH_SP_URL, { params: { sku, dm } }),
            ]);
    ...
    async function handleAddToCart(sp) {
        ...
        try {
            ...
            const { data, status } = await axios.post(DEALER_ADD_CART_URL, spToCartItem(sp, currentSku, headerModel));
            if (status === 200) {
                setSpList((list) =>
                    list.map((s) =>
                        s.spcode === sp.spcode
    ...
    async function handleQtyChange(sp, direction) {
        ...
        if (direction === "remove" && sp.cartQty <= 1) {
            ...
            try {
                await axios.delete(`/dealer-repair/orders/carts/delete/${sp.cartId}`);
                triggerCartRefresh();
            } catch (err) {
                showToast(err.response?.data?.message || "เกิดข้อผิดพลาด", "error");
                triggerCartRefresh();
            ...
            try {
                await axios.post(`/dealer-repair/orders/carts/add-remove/${direction}`, { id: sp.cartId });
                triggerCartRefresh();
            } catch (err) {
                showToast(err.response?.data?.message || "เกิดข้อผิดพลาด", "error");
                triggerCartRefresh();
    ...
    return (
        <AuthenticatedLayout>
            <Head title="สั่งซื้ออะไหล่ (Dealer)" />
            ...
            <Container maxWidth={false} sx={{ backgroundColor: "white", p: { xs: 2, md: 3 } }}>
                ...
                {/* ── Cart Section ─────────────────────────────────── */}
                <CartDrawer
                    cartJsonUrl={DEALER_CART_JSON_URL}
                    diagramRoute={DEALER_DIAGRAM_ROUTE}
                    storeUrl="/dealer-repair/orders/carts/store"
            ...
            >
                ...
                <DialogContent sx={{ p: 2 }}>
                    <Grid2 container spacing={2} sx={{ mt: 0 }}>
                        ...
                        <Grid2 size={{ xs: 12, md: 6 }}>
                            <Typography variant="caption" color="text.secondary" fontWeight="bold" sx={{ display: "block", mb: 1, mt: 2 }}>
                                วิธีดูหมายเลขซีเรียล (Serial Number)
                            </Typography>
                            <Box
                                component="img"
