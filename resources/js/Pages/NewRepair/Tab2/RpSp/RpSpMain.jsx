...
export default function RpSpMain({
  listSparePart,
  productDetail,
  setStepForm,
  JOB,
  ...
  // ตรวจสอบว่าเป็นการค้นหา 9999 
  const isSearchBy9999 = serial.startsWith("9999");

  // diagram layers ดิบจาก backend
  const memoDiagramLayers = useMemo(
    () => productDetail?.diagram_layers || [],
    [productDetail?.diagram_layers]
  );
  ...
  // ตรวจ combo set
  useEffect(() => {
    setShowComboDialog(productDetail?.is_combo || productDetail?.combo_set);
  }, [productDetail]);

  // โหลด model options (จริง ๆ คือ DM01 / DM02)
  useEffect(() => {
    const rawOptions = productDetail.model_options || [];
    const options = Array.isArray(rawOptions)
      ? rawOptions.map((m) => String(m).trim()).filter((m) => m)
    ...
    // เงื่อนไขเดียว: ให้เลือก DM ได้ ถ้ามีมากกว่า 1
    // if ((productDetail.allow_model_select || isSearchBy9999) && options.length > 0) {
    //   setModelOptions(options);
    //   setSelectedModel(options[0]); // default DM01
    // } else {
    //   setModelOptions([]);
    //   setSelectedModel(null);
    // }
    if (isSearchBy9999 && options.length > 0) {
      setModelOptions(options);
      setSelectedModel(options[0]); // default DM01
    } else {
      setModelOptions([]);
  ...
  // โหลดอะไหล่เก่าที่เลือกไว้
  useEffect(() => {
    fetchData().finally(() => setLoading(false));
  }, []);

  // รวม diagram ไม่ให้ซ้ำ
  const diagramLayers = useMemo(() => {
    const layers = memoDiagramLayers || [];
    const unique = [];
    const seen = new Set();
    ...
    layers.forEach((layer) => {
      const key = `${layer.path_file}_${layer.layout}_${layer.typedm}_${layer.modelfg}`;
      if (!seen.has(key)) {
        seen.add(key);
        unique.push(layer);
      }
  ...
  const Saved = (path = null) => {
    setStepForm(2);
    if (path) window.open(path, "_blank");
  };

  // กรองอะไหล่ตาม **Layout + DM**
  useEffect(() => {
    const normalizeLayout = (val) => {
      if (typeof val === "number") return val === 1 ? "outside" : "inside";
      const str = String(val).toLowerCase().trim();
    ...
    // 2) กรองตาม DM (typedm)
    // const byDM =
    //   selectedModel && (productDetail.allow_model_select || isSearchBy9999)
    //     ? byLayout.filter(
    //       (x) =>
    //         x.typedm &&
  ...
  return (
    <>
      {loading ? (
        ...
        <Grid2 container spacing={2}>
          {/* LEFT PANEL (DM SELECT + DIAGRAM) */}
          <Grid2 size={{ md: 3, sm: 12 }}>
            <Card sx={{ height: "100%", p: 1 }}>
              <CardContent sx={{ flex: 1, overflowY: "auto" }}>
                {/* {(productDetail.allow_model_select || isSearchBy9999) &&
                  modelOptions.length > 0 && ( */}
                {isSearchBy9999 && modelOptions.length > 0 && (
                  <Box sx={{ mb: 2 }}>
                    <Autocomplete
                      fullWidth
                      size="small"
                    ...
                    const normalized =
                      typeof layout === "number"
                        ? layout === 1
                          ? "outside"
                          : "inside"
                        : String(layout || "outside").trim().toLowerCase();
