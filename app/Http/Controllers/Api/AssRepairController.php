...
class AssRepairController extends Controller
{
    ...
    public function getRepairInfoBySerial(Request $request): JsonResponse
    {
        $validator = \Illuminate\Support\Facades\Validator::make($request->all(), [
            'serial_id' => 'required|string',
        ], [
            'serial_id.required' => 'กรุณาระบุ serial_id',
        ]);
        ...
        if ($jobs->isEmpty()) {
            return response()->json([
                'success' => false,
                'message' => 'ไม่พบข้อมูลงานซ่อมที่ยังไม่เสร็จสิ้นจาก serial_id ที่ระบุ',
            ], 404);
        }
