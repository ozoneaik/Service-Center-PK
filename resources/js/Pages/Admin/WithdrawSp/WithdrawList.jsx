...
export default function WithdrawList({ count_cart, message, sku, result, job_id, is_code_cust_id }) {
    ...
    const handleSearch = (e) => {
        ...
        router.get(
            route("withdrawSp.index"),
            { sku: searchValue },
            { replace: true, preserveScroll: true, onStart: () => setLoading(true), onFinish: () => setLoading(false) }
        );
    };
    ...
    // ✅ กรอง sp ตาม model ที่เลือก (DM01 / DM02)
    // useEffect(() => {
    //     if (!selectedModel) {
    //         setSpList(allSp);
    //         return;
    ...
    //     } else {
    //         // ถ้าเลือกชื่อโมเดลปกติ (เช่น J-12BID1504)
    //         filtered = allSp.filter((sp) => sp.modelfg === selectedModel);
    //     }

    //     const sorted = filtered.sort(sortByTracking);
    //     setSpList(sorted);
    // }, [selectedModel, allSp]);
                ...
                {loading ? (
                    ...
                    <Stack direction={{ xs: "column", md: "row" }} spacing={2} mt={3}>
                        ...
                        <Paper variant="outlined" sx={{ flex: 2, p: 2 }}>
                            <Box height={650} sx={{ overflowY: "auto" }}>
                                <WithdrawRowView spList={spList} onAdded={handleAdded} />
                            </Box>
                        </Paper>
                    </Stack>
                ) : (
